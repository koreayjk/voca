...
## 6. 앱스토어 iOS 앱은 웹 푸시가 아니라 APNs

**WKWebView 에는 ServiceWorker/PushManager 가 아예 없습니다.** 그래서 앱스토어로 배포한
iOS 앱에서는 위의 VAPID 웹 푸시가 동작하지 않고, 애플 서버(APNs)가 직접 기기로 보내야 합니다.

바뀌는 것은 **전송 방식 하나뿐**입니다. 누구에게·언제 보낼지(시간대·발송 시각·중복 방지·
밀린 개수·4개 국어 문구)는 웹과 완전히 같은 코드를 씁니다.

- 스키마: `supabase/push-ios-apns.sql` 실행 → `voca_push_subs.platform` 추가
- 전송: `supabase/functions/send-review-push/apns.ts`
- 설정 순서: **`docs/apple-app-store.md` 6번**
...
| 플랫폼 | 알림 | 아이콘 배지 | 방식 |
|---|---|---|---|
| 안드로이드 (Play 앱) | ✅ | OS 자동 — 런처에 따라 숫자 또는 점 | 웹 푸시(VAPID) |
| 아이폰 (앱스토어 앱) | ✅ | ✅ 숫자 | **APNs** |
| 아이폰 (Safari → 홈 화면에 추가, iOS 16.4+) | ✅ | ✅ 숫자 | 웹 푸시(VAPID) |
| PC (Chrome·Edge 설치형) | ✅ | ✅ 숫자 | 웹 푸시(VAPID) |
| 그냥 브라우저로 접속 | ✅ | ❌ (아이콘이 없으니 배지도 없음) | 웹 푸시(VAPID) |
