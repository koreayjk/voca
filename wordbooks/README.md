...
→ **gen_audio.py 예문 음원 파일명 = 문장해시 방식으로 수정(2026-08-14):** 기존 영어책과 단어가 겹쳐도(옵션B) 예문 음원이 섞이지 않도록, 예문 mp3를 `s/<safe-en>_<sentHash(ex)>.mp3`로 저장(앱 `_audioUrl`의 djb2 해시와 정확히 일치). 단어 발음(`w/<safe-en>.mp3`)은 어느 책이든 동일하므로 공유 유지. 이 수정으로 모든 책의 예문 음원이 문장 기준으로 유니크해져 과거의 `s/<단어>.mp3` 공유 충돌(예문 음성 불일치)이 구조적으로 사라짐.
